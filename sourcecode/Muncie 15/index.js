...
if(req.headers['csclient'] === "app") {
  ...
  value.push(usr + "-" + user)
  db.set('emails', value).then(() => {console.log('good')})
  }
  })
} else {
...
await rp({method: "POST", headers: {csclient: 'app', uname: uname[0]}, body: value, json: true, uri: url})
wait(60000);
})
}
...
app.get('/ai', function(req, res, next) {
  ...
  AI.generate(prompt, async (result) => {
    ...
    res.send('Error Generating. Redirecting. <script>window.location.replace("/")</script>')
    }
...
app.get('/edit', async (req, resp) => {
  ...
  async function final() {
    if (exports.e === null) {
      ...
      renderTemplate(resp, req, 'edit.ejs', { favs, uname, len, token, summary, name });
    }
  }
  final()
} else {
...
app.get("/search", async (req, res) => {
  ...
  var html = ""; renderTemplate(res, req, 'signin.ejs', {html})
...
app.get('/plzadd', async (req, res) => {
  ...
  db.get(token).then(value => {
    ...
    if (list === '[]') {
      ...
      list.push(name)
      db.set(token, list).then(() => { res.redirect('/plzesadd') });
    } else {
      list.push(name)
      db.set(token, list).then(() => { res.redirect('/plzesadd') });
    }
