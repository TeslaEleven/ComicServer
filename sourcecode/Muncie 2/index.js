...
const data = await fetch(`https://discord.com/api/users/@me`, { headers: { Authorization: `Bearer ${req.session.bearer_token}` } });
  ...
  var banner = exports.ban;
  var uname = exports.uname;
  var disc = exports.disc;
  var id = exports.id;
  var ava = exports.ava;
  var avatar = "https://cdn.discordapp.com/avatars/" + id + "/" + ava + "?size=512"
  if(ava === null) {
  ...
  const s9 = exports.s9
  renderTemplate(resp, req, 'index.ejs', { banner, uname, disc, id, avatar, s1, s2, s3, s4, s5, s6, s7, s8, s9 });
  } else {
    renderTemplate(resp, req, 'csdv.ejs');
  }
})

app.get('/csdv', async (req, resp) => {
  renderTemplate(resp, req, 'csdv.ejs');
})

app.get('/api', async (req, resp) => {
  renderTemplate(resp, req, 'info.ejs')
})
...
app.get('/cse', (req, res) => {
  //res.sendFile('/home/runner/ComicServer-Muncie/zeppelin/index.html');
  res.send('ComicServer Express v4 has been canceled.')
});

app.get('/muncie', async (req, resp) => {
  renderTemplate(resp, req, 'muncie.ejs')
})

app.get('/cesium', async (req, resp) => {
  renderTemplate(resp, req, 'cesium.ejs')
})
...
app.listen(3000, () => {
  console.log('ComicServer has Departed');
});