...
const ms = require('ms');
const passwordProtected = require('./pass-pro');
...
module.exports = async client => {
  ...
  const gcdate = date.getFullYear() + "/" + mo + "/" + date.getDate();
  const ckdate = date.getFullYear() + "-" + mo + "-" + date.getDate();

    const parsedPageGa = parse(
      await rp(
        `https://www.gocomics.com/garfield/${gcdate}`
      ).catch(err => {
        console.log('Request failed\n', err);
      })
    );
    const csImageGa = parsedPageGa
      .querySelector('.item-comic-image img')
      .rawAttrs.split(/ src=/)[1]
      .replace(/"/g, '');
    const csRawDateGa = parsedPageGa
      .querySelector('.item-comic-image img')
      .rawAttrs.split(/ alt=/)[1]
      .replace(/"/g, '');
    const csDateGa = csRawDateGa.slice(0, -70);

    const parsedPageBl = parse(await rp(`https://v7.comicskingdom.net/comics/blondie/${ckdate}`)
      .catch(err => {
        console.log("Request failed\n", err);
      })
    );
    const csRawBl = parsedPageBl.querySelector("div.col-12.text-center img");
    const csImageBl = htmlToText(csRawBl, {
      wordwrap: 130
    }).slice(1, -1);

    const parsedPagePop = parse(await rp(`https://v7.comicskingdom.net/comics/popeye/${ckdate}`)
      .catch(err => {
        console.log("Request failed\n", err);
      })
    )
    const csRawPop = parsedPagePop.querySelector("div.col-12.text-center img");
    const csImagePop = htmlToText(csRawPop, {
      wordwrap: 130
    }).slice(1, -1);

        const parsedPageHh = parse(await rp(`https://v7.comicskingdom.net/comics/hagar-the-horrible/${ckdate}`)
      .catch(err => {
        console.log("Request failed\n", err);
      })
    )
    const csRawHh = parsedPageHh.querySelector("div.col-12.text-center img");
    const csImageHh = htmlToText(csRawHh, {
      wordwrap: 130
    }).slice(1, -1);

        const parsedPageBb = parse(await rp(`https://v7.comicskingdom.net/comics/beetle-bailey-1/${ckdate}`)
      .catch(err => {
        console.log("Request failed\n", err);
      })
    )
    const csRawBb = parsedPageBb.querySelector("div.col-12.text-center img");
    const csImageBb = htmlToText(csRawBb, {
      wordwrap: 130
    }).slice(1, -1);

        const parsedPageGc = parse(
      await rp(
        `https://www.gocomics.com/garfield-classics/${gcdate}`
      ).catch(err => {
        console.log('Request failed\n', err);
      })
    );
    const csImageGc = parsedPageGc
      .querySelector('.item-comic-image img')
      .rawAttrs.split(/ src=/)[1]
      .replace(/"/g, '');
    const csRawDateGc = parsedPageGc
      .querySelector('.item-comic-image img')
      .rawAttrs.split(/ alt=/)[1]
      .replace(/"/g, '');
    const csDateGc = csRawDateGc.slice(0, -70);

          const parsedPageBn = parse(
      await rp(
        `https://www.gocomics.com/bignate/${gcdate}`
      ).catch(err => {
        console.log('Request failed\n', err);
      })
    );
    const csImageBn = parsedPageBn
      .querySelector('.item-comic-image img')
      .rawAttrs.split(/ src=/)[1]
      .replace(/"/g, '');
    const csRawDateBn = parsedPageBn
      .querySelector('.item-comic-image img')
      .rawAttrs.split(/ alt=/)[1]
      .replace(/"/g, '');
    const csDateBn = csRawDateBn.slice(0, -70);
  
      const parsedPagePe = parse(
      await rp(
        `https://www.gocomics.com/peanuts/${gcdate}`
      ).catch(err => {
        console.log('Request failed\n', err);
      })
    );
    const csImagePe = parsedPagePe
      .querySelector('.item-comic-image img')
      .rawAttrs.split(/ src=/)[1]
      .replace(/"/g, '');
    const csRawDatePe = parsedPagePe
      .querySelector('.item-comic-image img')
      .rawAttrs.split(/ alt=/)[1]
      .replace(/"/g, '');
    const csDatePe = csRawDatePe.slice(0, -70);
  renderTemplate(res, req, 'favorites.ejs', { csImageGa, csDateGa, csImageBl, ckdate, csImagePop, csImageHh, csImageBb, csImageGa, csDateGa, csImageGc, csDateGc, csImageBn, csDateBn, csImagePe, csDatePe })
});
  ...
  app.get('/old/oldversion', (req, res) => {
    renderTemplate(res, req, 'oldversion.ejs');
  });
