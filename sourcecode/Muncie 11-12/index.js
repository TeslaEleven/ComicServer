...
app.get('/m11', async function(req, resp, next) {
    if (req.user) {
  ...
  async function final() {
    if (exports.e === null) {
      ...
      renderTemplate(resp, req, 'nextgen.ejs', { favs, uname });
    }
  }
  final()
  } else {
      const favs = ["Blondie", "Garfield"]
      const uname = "Test Account v1"
      renderTemplate(resp, req, 'nextgen.ejs', { favs, uname });
  }
});
...
app.get('/watch', async (req, resp) => {
  const token = "csadmin124"
  await db.get(token).then(value => { exports.e = value });
      const uname = token
      await db.get(token).then(value => { exports.favs = value });
      const favs = ["Blondie", "Popeye", "Hagar the Horrible", "Beetle Bailey", "Garfield", "Garfield-Classics", "Big Nate", "Peanuts"]
      renderTemplate(resp, req, 'watchdex.ejs', { favs, uname });
})
...
app.get('/renderguest', cors(), async (req, resp) => {
const uname = "csapp-6.9.0-stable"
db.get("csadmin124").then(value => {exports.ee = value})
const favs = exports.ee
renderTemplate(resp, req, 'index.ejs', { favs, uname });
})
...
app.get('/edit', async (req, resp) => {
  ...
  async function final() {
    if (exports.e === null) {
      ...
      renderTemplate(resp, req, 'editt.ejs', { favs, uname, len, token });
    }
  }
  final()
} else {
